...
void processBoundaryFixing(stringstream &instream, string tileID, stringstream &outstream, stringstream &formatedStream)
{	
	vector<points> coord;
...
//					cout << "# " << i << " output size " << poly_out[i].size() << endl;
					if (poly_out[i].size()>=3)
					{
						...
						//cout << dsv(cwcp,","," ","","","","") << endl; 
						formatedStream << tileID << "\t" << rec_id << "\t" << dsv(cwcp,","," ","","","","") << endl;
					}
				}
			}
