...
static char tiledir[256] = "/home/cloudera/Downloads/tiles";
static char imagedir[256] = "/home/cloudera/Downloads/images";
...
static int64_t tileWidth = 4096;
static int64_t tileOverlap = 5;
...
void *thread_func(void *arg)
{
	while(1)
	{
		...
		int64_t w = tileWidth, h = tileWidth;
...
int main(int argc, char **args) {
	...
	if(argc>1)
	{
		strcpy(imagedir,args[1]); // input dir
		strcpy(tiledir,args[2]);  // output dir
		tileWidth = atoi(args[3]);    
		tileOverlap = atoi(args[4]);
		threadNum = atoi(args[5]);
	}
