...
void processBoundaryFixing2(stringstream &instream, string tileID, int baseX, int baseY)
{
	vector<points> coord;
...
//							cout << j+1 << " 4del " << coord2[j+1].x << "," << coord2[j+1].y << endl;
			...
			if (proc)
			{
				...
				/* Perform join with the inner tile */
				for (t=0;t<ctr1;t++)
				{
					{
						cout << " " << baseX + coord2[t].x << "," << baseY + coord2[t].y;
					}
				}
